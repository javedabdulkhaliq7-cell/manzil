  ...
  if (mcqs.length === 0 && blanks.length === 0) {
    return (
      <div className="flex flex-col h-screen items-center justify-center bg-gray-50 gap-3 px-4">
        <div className="text-sm text-gray-400 text-center">Quiz content for this chapter isn't ready yet.</div>
    ...
    return (
      <div className="flex flex-col h-screen bg-gray-50">
        <div className="bg-gradient-to-br from-emerald-700 to-emerald-500 px-4 pt-8 pb-8 text-white flex-shrink-0">
          ...
          <p className="text-emerald-100 text-sm mt-1">MCQs + Fill in the Blanks, one fast round</p>
        </div>
        <div className="flex-1 px-4 py-6 flex flex-col gap-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-sm p-4">
            <div className="text-xs font-bold text-gray-400 uppercase mb-3">What's in this quiz</div>
            <div className="flex flex-col gap-2 text-xs">
              <div className="flex justify-between"><span className="text-gray-600">Part 1 — MCQs</span><span className="font-bold text-slate-900">{mcqs.length} questions</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Part 2 — Fill in the Blanks</span><span className="font-bold text-slate-900">{blanks.length} questions</span></div>
              <div className="flex justify-between border-t border-gray-100 pt-2 mt-1"><span className="font-bold text-slate-900">Total</span><span className="font-black text-emerald-600">{maxScore} points · {CONFIG.TIME_MINUTES} min</span></div>
            </div>
          </div>
        ...
        <div className="px-4 py-3 bg-white border-t border-gray-100 flex-shrink-0">
          <button onClick={() => setPhase(mcqs.length > 0 ? 'mcq' : 'fib')} className="w-full bg-gradient-to-r from-emerald-700 to-emerald-500 text-white font-bold py-4 rounded-2xl text-sm shadow-lg shadow-emerald-200 active:scale-95 transition-all">
            Start Quick Quiz ▶
          </button>
        </div>
      </div>
    ...
    return (
      <div className="flex flex-col h-screen bg-gray-50">
        <div className="bg-gradient-to-br from-emerald-700 to-emerald-500 px-4 py-3 text-white flex-shrink-0">
          ...
          <div className="text-xs text-emerald-100">{answeredCount} of {mcqs.length} answered</div>
        </div>
        ...
        <div className="px-4 py-3 flex gap-3 bg-white border-t border-gray-100 flex-shrink-0">
          ...
          {mcqIndex + 1 < mcqs.length ? (
            ...
            <button onClick={() => setPhase(blanks.length > 0 ? 'fib' : 'results')} className="flex-1 bg-gradient-to-r from-emerald-700 to-emerald-500 text-white font-bold py-3 rounded-2xl text-sm shadow-lg shadow-emerald-200 active:scale-95 transition-all">
    ...
    return (
      <div className="flex flex-col h-screen bg-gray-50">
        <div className="bg-gradient-to-br from-emerald-700 to-emerald-500 px-4 py-3 text-white flex-shrink-0">
          ...
          <div className="text-xs text-emerald-100">Answer all {blanks.length}</div>
        </div>
        <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
          {blanks.map((b, i) => (
            <div key={b.id} className="bg-white rounded-2xl shadow-sm p-4 border border-gray-100">
              <div className="text-[10px] text-gray-400 font-semibold mb-1">Q{i + 1}</div>
              <p className="text-sm font-semibold text-slate-900 mb-2"><FractionText text={b.question} /></p>
              <input
        ...
        <div className="px-4 py-3 flex gap-3 bg-white border-t border-gray-100 flex-shrink-0">
          ...
          <button onClick={submitQuiz} className="flex-1 bg-gradient-to-r from-emerald-700 to-emerald-500 text-white font-bold py-3 rounded-2xl text-sm shadow-lg shadow-emerald-200 active:scale-95 transition-all">Submit Quiz ✓</button>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="flex flex-col h-screen bg-gray-50 overflow-y-auto">
        ...
        <div className="p-4 flex flex-col gap-4">
          <div className="bg-white rounded-2xl shadow-sm p-4">
            <div className="text-xs font-bold text-gray-400 uppercase mb-3">Breakdown</div>
            ...
            <div className="flex flex-col gap-2">
              {results.mcqBreakdown.map((m, i) => (
                <div key={i} className="border-b border-gray-50 pb-2 last:border-0">
                  ...
                  <div className={`text-[10px] font-bold mt-0.5 ${m.correct ? 'text-emerald-600' : 'text-red-500'}`}>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-4">
            <div className="text-xs font-bold text-gray-400 uppercase mb-3">Fill in the Blanks Review</div>
            <div className="flex flex-col gap-2">
              {results.blankBreakdown.map((b, i) => (
                <div key={i} className="border-b border-gray-50 pb-2 last:border-0">
                  <div className="text-xs font-semibold text-slate-800"><FractionText text={b.question} /></div>
                  <div className={`text-[10px] font-bold mt-0.5 ${b.correct ? 'text-emerald-600' : 'text-red-500'}`}>
                    {b.correct ? (
                      <>✓ Correct: {b.typed}</>
                    ) : (
