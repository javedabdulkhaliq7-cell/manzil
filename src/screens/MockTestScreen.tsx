    ...
    return (
      <div className="flex flex-col h-screen bg-gray-50">
        <div className="bg-gradient-to-br from-emerald-700 to-emerald-500 px-4 pt-8 pb-8 text-white flex-shrink-0">
          ...
          <p className="text-emerald-100 text-sm mt-1">{subject ? `${subject.name} · Balochistan Board` : 'Full Syllabus · Balochistan Board'}</p>
        </div>
        <div className="flex-1 px-4 py-6 flex flex-col gap-4">
          <div className="bg-white rounded-2xl shadow-sm p-4">
            <div className="text-xs font-bold text-gray-400 uppercase mb-3">Test Info</div>
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-br from-emerald-700 to-emerald-500 px-4 py-3 text-white flex-shrink-0">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <div className="text-xs text-emerald-100">{subject ? subject.name : 'Full Syllabus'}</div>
          </div>
      ...
      <div className="h-1 bg-gray-100 flex-shrink-0">
        <div className="h-full bg-gradient-to-r from-emerald-600 to-emerald-400 transition-all" style={{ width: `${((current + 1) / mcqs.length) * 100}%` }} />
      </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 border-2 rounded-2xl px-4 py-3 text-sm text-left transition-all active:scale-[0.99] ${
                  isChosen ? 'border-emerald-400 bg-emerald-50 text-emerald-800' : 'border-gray-200 bg-white text-gray-700'
                }`}
              >
          ...
          {mcqs.map((_, i) => (
            <button
              ...
              className={`w-7 h-7 rounded-lg text-[10px] font-bold transition-all ${
                i === current ? 'bg-slate-900 text-emerald-400' :
                answered[i] ? 'bg-emerald-500 text-white' :
      ...
      <div className="px-4 py-3 flex gap-3 bg-white border-t border-gray-100 flex-shrink-0">
        <button
          ...
          disabled={current === 0}
          className="flex-1 border-2 border-gray-200 text-gray-500 font-bold py-3 rounded-2xl text-sm disabled:opacity-40 active:scale-95 transition-all"
        >
          ← Previous
        </button>
        {current + 1 < mcqs.length ? (
          <button
            ...
            className="flex-1 bg-gradient-to-r from-emerald-700 to-emerald-500 text-white font-bold py-3 rounded-2xl text-sm shadow-lg shadow-emerald-200 active:scale-95 transition-all"
          >
            Next →
          </button>
        ) : (
